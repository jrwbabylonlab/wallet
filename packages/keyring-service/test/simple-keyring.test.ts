...
describe('bitcoin-simple-keyring', () => {
  ...
  describe('#sign psbt', function () {
    it('sign P2TR input', async function () {
      ...
      await newKeyring.signTransaction(psbt, [{ index: 0, publicKey: pubkey }], {
    ...
    it('sign P2TR multisig input', async function () {
      ...
      const accounts = await newKeyring.getAccounts()
      const pubkeys = accounts.map((hex: string) => Buffer.from(hex, 'hex'))
    ...
    it('disableTweakSignerForLegacyInput', async function () {
      ...
      const keyPair = eccManager.eccPair.fromPrivateKey(Buffer.from(testAccount.key, 'hex'), {
        network,
      })
      ...
      await originKeyring.signTransaction(
        psbt,
        [{ index: 0, publicKey: originPubkey, useTweakedSigner: true }],
        {
          network: bitcoin.networks.bitcoin,
        }
      )
