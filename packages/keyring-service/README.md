...
- `deriveContextHash(publicKey: string, appName: string, context: string): Promise<string>` - Derive a deterministic 32-byte value via HKDF-SHA-256 (RFC 5869). For mnemonic-imported HD keyrings, IKM is the BIP-32 privkey at the sibling path `m/73681862'/coin'/account'/change/index` (per spec v2.0). For xpriv-imported HD keyrings and raw-imported simple keyrings, IKM is the HMAC-SHA-512 wrap of the connected privkey under label `"derive-context-hash-from-k"`. The `appName` must be 1-64 bytes, lowercase `[a-z0-9\-]`.
