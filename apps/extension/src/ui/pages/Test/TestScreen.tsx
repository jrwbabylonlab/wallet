import { useEffect, useRef } from 'react';

import { Button, Card, Column, Content, Header, Input, Layout, Text } from '@/ui/components';
import { AddressDetailPopover } from '@/ui/components/AddressDetailPopover';
import { AddressText } from '@/ui/components/AddressText';
import { AddressTypeCard2 } from '@/ui/components/AddressTypeCard';
import { RemoveWalletPopover } from '@/ui/components/RemoveWalletPopover';
import { SignPsbtWithRisksPopover } from '@/ui/components/SignPsbtWithRisksPopover';
import { ProviderMethodArgs, ProviderMethods } from '@unisat/wallet-background/types';
import { ADDRESS_TYPES, DummyTxType, SignMessageType, ToAddressInfo, ToSignData } from '@unisat/wallet-shared';
import { useAsyncEffect, useCurrentKeyring, useKeyrings, useTools, useWallet } from '@unisat/wallet-state';
import { ChainType, NetworkType } from '@unisat/wallet-types';
import { Connect, SignMessage, SignPsbt, SwitchNetwork } from '../Approval/components';
import { useNavigate } from '../MainRoute';
import { testDecodedTx } from './testDecodedTx';

const DUMMY_SESSION = {
  origin: 'https://mail.google.com/',
  icon: 'https://ssl.gstatic.com/ui/v1/icons/mail/rfr/gmail.ico',
  name: 'GMAIL'
};

export default function TestScreen() {
  const tools = useTools();
  const wallet = useWallet();

  const testApprovalData = useRef<{ toSignDatas: ToSignData[] }>({ toSignDatas: [] });

  useAsyncEffect(async () => {
    const toSignDatas: ToSignData[] = [];

    const types = [DummyTxType.SEND_BTC, DummyTxType.SPLIT_BTC, DummyTxType.MERGE_BTC, DummyTxType.LIST_ASSETS];
    for (const txType of types) {
      const data = await wallet.createDummyPsbt({ txType });
      toSignDatas.push(data);
    }

    testApprovalData.current.toSignDatas = toSignDatas;
  });

  const requestApproval = async (data: ProviderMethodArgs<ProviderMethods>) => {
    try {
      await wallet.requestMethod({
        data,
        session: DUMMY_SESSION
      });
      tools.toast('Request sent');
    } catch (e) {
      tools.toast('Error sending request');
      console.log('Error sending request', e);
    }
  };

  return (
    <Layout>
      <Header title="TestScreen" />
      <Content>
        <Column>
          <Button
            text="SignPsbt"
            onClick={() => {
              requestApproval({
                method: 'signPsbt',
                params: {
                  psbtHex: testApprovalData.current.toSignDatas[0].psbtHex,
                  options: { toSignInputs: testApprovalData.current.toSignDatas[0].toSignInputs }
                }
              });
            }}
          />

          <Button
            text="Sign Multiple Psbts"
            onClick={() => {
              requestApproval({
                method: 'multiSignPsbt',
                params: {
                  psbtHexs: testApprovalData.current.toSignDatas.map((item) => item.psbtHex),
                  options: testApprovalData.current.toSignDatas.map((item) => ({ toSignInputs: item.toSignInputs }))
                }
              });
            }}
          />

          <Button
            text="SignMessage"
            onClick={() => {
              requestApproval({
                method: 'signMessage',
                params: {
                  text: 'hello',
                  type: SignMessageType.ECDSA
                }
              });
            }}
          />
          <Button
            text="Sign Multiple Messages"
            onClick={() => {
              requestApproval({
                method: 'multiSignMessage',
                params: {
                  messages: [
                    {
                      text: 'hello1',
                      type: SignMessageType.BIP322_SIMPLE
                    },
                    {
                      text: 'hello2',
                      type: SignMessageType.ECDSA
                    }
                  ]
                }
              });
            }}
          />

          <Button
            text="Switch Chain"
            onClick={() => {
              requestApproval({
                method: 'switchChain',
                params: {
                  chain: ChainType.BITCOIN_MAINNET
                }
              });
            }}
          />
        </Column>
      </Content>
    </Layout>
  );
}

function TestWalletAccounts() {
  const wallet = useWallet();
  const tools = useTools();

  const keyrings = useKeyrings();
  return (
    <Layout>
      <Header />
      <Content>
        <Column>
          <Text text="Create 100 wallets for performance test" />
          <Text text={`Current keyrings count: ${keyrings.length}`} />
          <Button
            text="Create multiple wallets"
            onClick={async () => {
              const count = 100;
              const addressTypeInfo = ADDRESS_TYPES[0];
              for (let i = 0; i < count; i++) {
                const mnemonics = await wallet.generatePreMnemonic();
                await wallet.createKeyringWithMnemonics(
                  mnemonics,
                  addressTypeInfo.hdPath,
                  '',
                  addressTypeInfo.value,
                  10
                );
              }
              tools.toastSuccess(`Created ${count} wallets`);
            }}
          />
        </Column>
      </Content>
    </Layout>
  );
}

function TestSignPsbtWithRisksPopover() {
  return (
    <SignPsbtWithRisksPopover
      onClose={() => {
        // todo
      }}
      onConfirm={() => {
        // todo
      }}
      decodedPsbt={testDecodedTx.data as any}
    />
  );
}

const testAddressInfo = {
  address: 'tb1q8h8s4zd9y0lkrx334aqnj4ykqs220ss7mjxzny',
  domain: 'abc.sats',
  inscription: {
    inscriptionId: '8436ee125d5cff80292e1ae3efdcde9880f020b63c5ee0f08a39865fc534ec36i0',
    inscriptionNumber: 1254,
    address: 'tb1q8h8s4zd9y0lkrx334aqnj4ykqs220ss7mjxzny',
    outputValue: 546,
    preview: 'https://ordinals.com/preview/8436ee125d5cff80292e1ae3efdcde9880f020b63c5ee0f08a39865fc534ec36i0',
    content: 'https://ordinals.com/content/8436ee125d5cff80292e1ae3efdcde9880f020b63c5ee0f08a39865fc534ec36i0',
    contentLength: 40,
    contentType: 'text/plain;charset=utf-8',
    contentBody: '',
    timestamp: 1680586687,
    genesisTransaction: '8436ee125d5cff80292e1ae3efdcde9880f020b63c5ee0f08a39865fc534ec36',
    location: '8436ee125d5cff80292e1ae3efdcde9880f020b63c5ee0f08a39865fc534ec36:0:0',
    output: '8436ee125d5cff80292e1ae3efdcde9880f020b63c5ee0f08a39865fc534ec36:0',
    offset: 0
  }
};

function TestAddressInput() {
  const toInfo = {
    address: '',
    domain: ''
  };
  return (
    <Input
      preset="address"
      addressInputData={toInfo}
      onAddressInputChange={(val) => {
        console.log(val);
      }}
      autoFocus={true}
    />
  );
}

function TestButton() {
  return (
    <Card
      onClick={() => {
        //tpdp
      }}>
      <Text text="HELLO" />
      <Text text="WORLD" />
    </Card>
  );
}

function TestAddressDetail() {
  return (
    <AddressDetailPopover
      address="tb1q8h8s4zd9y0lkrx334aqnj4ykqs220ss7mjxzny"
      onClose={() => {
        //todo
      }}
    />
  );
}

function TestAddressTextA() {
  return <AddressText addressInfo={testAddressInfo as ToAddressInfo} />;
}

function TestAddressTextB() {
  return <AddressText address={'tb1q8h8s4zd9y0lkrx334aqnj4ykqs220ss7mjxzny'} />;
}

function TestApprovalConnect() {
  return (
    <Connect
      params={{
        session: {
          origin: 'https://mail.google.com/',
          icon: 'https://ssl.gstatic.com/ui/v1/icons/mail/rfr/gmail.ico',
          name: 'GMAIL'
        }
      }}
    />
  );
}

function TestApprovalSignPsbtB() {
  // const psbtHex = '';

  const psbtHex =
    '';
  return (
    <SignPsbt
      params={{
        data: {
          toSignDatas: [
            {
              psbtHex,
              toSignInputs: []
            }
          ]
        },
        session: {
          origin: 'https://mail.google.com/',
          icon: 'https://ssl.gstatic.com/ui/v1/icons/mail/rfr/gmail.ico',
          name: 'GMAIL'
        }
      }}
    />
  );
}

// function TestApprovalMultiSignPsbt() {
//   // const psbtHex = '';

//   const psbtHex =
//     '';
//   const psbtHex2 =
//     '';
//   return (
//     <MultiSignPsbt
//       params={{
//         data: {
//           psbtHexs: [psbtHex, psbtHex2],
//           options: []
//         },
//         session: {
//           origin: 'https://mail.google.com/',
//           icon: 'https://ssl.gstatic.com/ui/v1/icons/mail/rfr/gmail.ico',
//           name: 'GMAIL'
//         }
//       }}
//     />
//   );
// }

function TestApprovalSwitchNetwork() {
  return (
    <SwitchNetwork
      params={{
        data: {
          networkType: NetworkType.MAINNET
        },
        session: {
          origin: 'https://mail.google.com/',
          icon: 'https://ssl.gstatic.com/ui/v1/icons/mail/rfr/gmail.ico',
          name: 'GMAIL'
        }
      }}
    />
  );
}

function TestApprovalSignText() {
  return (
    <SignMessage
      params={{
        data: {
          toSignMessages: [{ text: 'hello', type: SignMessageType.ECDSA }]
        },
        session: DUMMY_SESSION
      }}
    />
  );
}

function TestRemoveWalletPoper() {
  const keyring = useCurrentKeyring();
  return (
    <RemoveWalletPopover
      keyring={keyring}
      onClose={function (): void {
        throw new Error('Function not implemented.');
      }}
    />
  );
}

function TestCreatePassword() {
  const navigate = useNavigate();
  useEffect(() => {
    navigate('CreatePasswordScreen', { isNewAccount: true });
  }, []);
}

function TestTxSuccess() {
  const navigate = useNavigate();
  useEffect(() => {
    setTimeout(() => {
      navigate('TxSuccessScreen', { txid: '' });
    }, 3000);
  }, []);
  return <div />;
}

function TestTxFailed() {
  const navigate = useNavigate();
  useEffect(() => {
    setTimeout(() => {
      navigate('TxFailScreen', { error: '!dfjksdfkds' });
    }, 3000);
  }, []);
  return <div />;
}

function TestAddressTypeCard() {
  const items = [
    { address: 'bc1qm72q6wrjy05h9fhu9gury67356nrcnvdd6853x', path: "m/84'/0'/0'/0/0", satoshis: 100 }
    // { address: 'bc1qug97mq778xeusw66z559uuwcyt7kxsmarrvl90', path: "m/84'/0'/0'/0/1" }
  ];
  return (
    <Layout>
      <Content>
        <Column my="md" mt="md">
          <AddressTypeCard2 label="Native Segwit (P2WPKH)" items={items} checked />
          <AddressTypeCard2 label="Nested Segwit (P2SH-P2WPKH)" items={items} checked={false} />
          <AddressTypeCard2 label="Taproot (P2TR)" items={items} checked={false} />
          <AddressTypeCard2 label="Legacy (P2PKH)" items={items} checked={false} />
        </Column>
      </Content>
    </Layout>
  );
}
