...
- `cursor` - `number`: (optional) pagination offset, default `0`
...
- `Promise` - `object[]`:
  ...
  - `scriptPk` - `string`: scriptPubKey hex
  - `addressType` - `number`: address type enum
  - `inscriptions` - `object[]`: inscriptions on this UTXO (should be empty for spendable UTXOs)
