...
- **`unisat.deriveContextHash` IKM source changed (spec v1.0 → v2.0).** Mnemonic-imported HD wallets now derive IKM at the sibling path `m/73681862'/coin'/account'/change/address_index` (the connected user-leaf path with its purpose segment replaced by `73681862'`). xpriv-imported HD wallets and imported raw private keys derive IKM via HMAC-SHA-512 wrap of the connected private key (BIP-85 pattern). The user's spending key is never used as direct HKDF input.
- **Per-position semantics.** Output rotates with the `(coin_type, account, change, address_index)` quad of the connected user leaf. Switching only the BIP-43 purpose / address type (e.g. BIP-44 ↔ BIP-86) at the same `(coin, account, change, index)` does NOT rotate the output by itself — apps that need per-script-type rotation MUST encode that in `context`. Switching mainnet ↔ testnet does not rotate either (UniSat uses `coin_type = 0` for both); applications that need network-bound outputs MUST encode the network in `context`.
- **Mnemonic vs xpriv interop.** Wallets restored via mnemonic and via sub-path xpriv import produce DIFFERENT outputs even from the same recovery phrase. Mnemonic-imported wallets use sibling-path derivation; xpriv-imported wallets use HMAC-wrap (no master access). dApps that persisted v1.0 outputs must re-derive against v2.0 — there is no version-discovery mechanism. The API remains marked `@experimental`. See [`../api/derive-context-hash.md`](../api/derive-context-hash.md) for the updated contract.
